function showWeatherWidget(msg) {
  document.getElementById('weather-widget').innerHTML = msg;
}
    ...
    fetch(`https://nominatim.openstreetmap.org/reverse?lat=${lat}&lon=${lon}&format=json`)
      ...
      .then(locationData => {
        ...
        fetch(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current=temperature_2m,wind_speed_10m,relative_humidity_2m,weather_code`)
          ...
          .catch(() => {
            showWeatherWidget('Không lấy được thời tiết');
          });
      })
      .catch(() => {
        showWeatherWidget('Không lấy được vị trí địa chỉ');
      });
  }, function() {
    showWeatherWidget('Không lấy được vị trí thiết bị');
  });
} else {
  showWeatherWidget('Trình duyệt không hỗ trợ định vị');
}
